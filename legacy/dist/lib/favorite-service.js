export const FavoriteService = {
     ...
     */
    filterFavoritesByProject(favorites, projectId) {
     ...
     */
    filterFavoritesByWorkspace(favorites, _workspaceId) {
     ...
     */
    findFavoriteByDescriptionOrId(favorites, input) {
        ...
        const matches = favorites.filter(f => f.description !== undefined && f.description !== null && f.description.toLowerCase().includes(lowercaseInput));
        if (matches.length === 0) {
            return null;
        }
        if (matches.length === 1) {
        ...
        const exactMatch = matches.find(f => f.description !== undefined && f.description !== null && f.description.toLowerCase() === lowercaseInput);
        if (exactMatch) {
            return exactMatch;
        }
     ...
     */
    findFavoriteById(favorites, favoriteId) {
     ...
     */
    async getFavorites(client, context) {
        try {
     ...
     */
    async getFavoritesForProject(client, projectId, context) {
        try {
     ...
     */
    async getFavoritesForWorkspace(client, workspaceId, context) {
        try {
            // Call the client API directly to ensure errors are propagated
     ...
     */
    async getFavoriteStats(client, context) {
        try {
            // Call the client API directly to ensure errors are propagated
     ...
     */
    async selectFavorite(client, input, context) {
        try {
     ...
     */
    async validateFavorite(client, favoriteId, context) {
