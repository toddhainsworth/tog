...
export class ProjectTaskSelector {
    ...
    findProjectByNameOrId(input) {
        ...
        const matches = this.projects.filter(p => p.name.toLowerCase().includes(lowercaseInput));
        if (matches.length === 0) {
            return null;
        }
        if (matches.length === 1) {
     ...
     * Supports exact matches taking precedence over partial matches.
     */
    findTaskByNameOrId(input, projectId) {
        // Try to parse as ID first
        const id = Number.parseInt(input, 10);
        if (!Number.isNaN(id)) {
     ...
     */
    async selectProjectAndTask(flags) {
        let selectedProject;
        let selectedTask;
        // Project selection
        if (flags.project) {
     ...
     */
    async selectProjectByFlag(projectFlag) {
     ...
     */
    async selectTaskByFlag(taskFlag, selectedProject) {
