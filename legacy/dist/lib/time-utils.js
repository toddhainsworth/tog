...
dayjs.extend(utc);
import { DEFAULT_TIME_FORMAT, MILLISECONDS_PER_SECOND, SECONDS_PER_HOUR, SECONDS_PER_MINUTE, TIME_FORMAT_PAD_CHAR, TIME_FORMAT_PADDING_LENGTH, TOGGL_FOUNDED_DATE } from './constants.js';
...
export function getPastDaysDateRange(daysAgo) {
 ...
 */
export function getAllTimeSearchRange() {
    return {
...
export function formatWeekRange(dateRange) {
...
export function aggregateTimeEntriesByProject(entries, projects = []) {
...
export function calculatePercentage(projectSeconds, totalSeconds) {
    if (totalSeconds === 0)
        return 0;
    return Math.round((projectSeconds / totalSeconds) * 100);
}
export function groupTimeEntriesByDay(entries, projects = []) {
...
export function aggregateWeeklyProjectSummary(entries, projects = []) {
    const projectMap = new Map();
    let totalSeconds = 0;
