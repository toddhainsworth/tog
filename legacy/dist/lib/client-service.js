export const ClientService = {
     ...
     */
    createClientLookupMap(clients) {
        return new Map(clients.map(client => [client.name.toLowerCase(), client]));
    },
    /**
     * Filters clients by name pattern.
     */
    filterClientsByName(clients, pattern) {
        const lowercasePattern = pattern.toLowerCase();
        return clients.filter(c => c.name.toLowerCase().includes(lowercasePattern));
    },
    /**
     ...
     */
    findClientByNameOrId(clients, input) {
        ...
        const matches = clients.filter(c => c.name.toLowerCase().includes(lowercaseInput));
        if (matches.length === 0) {
            return null;
        }
        if (matches.length === 1) {
        ...
        const exactMatch = matches.find(c => c.name.toLowerCase() === lowercaseInput);
        if (exactMatch) {
            return exactMatch;
        }
     ...
     */
    getClientFromLookupMap(lookupMap, clientName) {
     ...
     */
    async getClients(client, context) {
        try {
     ...
     */
    async getClientStats(togglClient, projects, context) {
        try {
     ...
     */
    getClientsWithProjectCounts(clients, projects) {
     ...
     */
    async selectClient(togglClient, input, context) {
        try {
     ...
     */
    async validateClient(togglClient, clientId, context) {
