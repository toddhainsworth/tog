export const WorkspaceService = {
     ...
     */
    findWorkspaceByNameOrId(workspaces, input) {
        ...
        const matches = workspaces.filter(w => w.name.toLowerCase().includes(lowercaseInput));
        if (matches.length === 0) {
            return null;
        }
        if (matches.length === 1) {
        ...
        const exactMatch = matches.find(w => w.name.toLowerCase() === lowercaseInput);
        if (exactMatch) {
            return exactMatch;
        }
     ...
     */
    async getDefaultWorkspace(client, context) {
     ...
     */
    async getWorkspaces(client, context) {
        try {
     ...
     */
    async validateWorkspace(client, workspaceId, context) {
     ...
     */
    async validateWorkspaceAccess(client, workspaceId, context) {
